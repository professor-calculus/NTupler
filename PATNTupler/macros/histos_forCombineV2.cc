...
int main(){
    ...
    for (size_t iHt = 0; iHt < ht_bins.size(); ++iHt){ 
        ...
        // QCD ESTIMATION
        void doQcdHistograms(const unsigned int& numberOfBins; const std::vector<double>& data_binContentUnD, const std::vector<double>& data_binErrorUnD, const std::vector<std::vector<double>>& mcVec_binContentUnD, const std::vector<std::vector<double>>& mcVec_binErrorUnD){

            // first check all the vectors are the right size
            ...
            h_QCD = new TH1D("QCD", ";bin;events / bin", numberOfBins, 0, numberOfBins);
            std::map<std::string, TH1D*> hErr_;
        ...
        for (unsigned int iBin = 1; iBin < numberOfBins + 1; ++iBin){
            ...
            unsigned int iVec = iBin - 1;
            double ratio = QcdSidebandCorr::GetCorr(HT, iBin);
