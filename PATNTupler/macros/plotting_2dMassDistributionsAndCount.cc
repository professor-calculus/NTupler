...
int main(){
    ...
    // ONE: save info
    std::string outputDir = "/opt/ppd/scratch/xap79297/Analysis_boostedNmssmHiggs/plots_2017_07_05/MassCutsV02/QCD/invBTagV2_NOAK4/"; // where we are going to save the output plots (should include the samples name + binning maybe)
    ...
    for (size_t iMassRegion = 0; iMassRegion!=MassCutsObject.GetAllCuts().size()+1; ++iMassRegion){
        ...
        if (true){ // select which cuts we want to use
        // if (iMassRegion == MassCutsObject.GetAllCuts().size()){ // select which cuts we want to use
