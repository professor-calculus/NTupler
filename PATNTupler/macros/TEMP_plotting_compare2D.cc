...
int main(int argc, char** argv){
    ...
    //std::vector<std::vector<int>> cut6_lepVeto = { {0, 1}, {0, 1}, {0, 99999} }; // Lepton Veto BIN
    //std::vector<double> cut6_jetMhtDPhi = { 0.0, 0.0, 0.0 }; // Lepton Veto BIN
    ...
    for (size_t iCut2 = 0; iCut2 < cut2_ak8Dbt.size(); ++iCut2){
        for (size_t iCut3 = 0; iCut3 < cut3_ak8Pt.size(); ++iCut3){
            for (size_t iCut4 = 0; iCut4 < cut4_ht.size(); ++iCut4){
                for (size_t iCut6 = 0; iCut6 < cut6_mht.size(); ++iCut6){        
                    for (size_t iCut5 = 0; iCut5 < cut5_ak4Pt.size(); ++iCut5){
                        ...
                        std::string cutToApply = Form("%s && fatJetA_p4.Pt()>%d && fatJetB_p4.Pt()>%d && ht>=%d && ht<%d && mht>=%d && mht<%d && nrVetoObjects>=%d && nrVetoObjects<%d && jet_mht_dPhi>%f && slimJetA_p4.Pt()>%d && slimJetB_p4.Pt()>%d", dbtCut.c_str(), cut3_ak8Pt[iCut3], cut3_ak8Pt[iCut3], cut4_ht[iCut4][0], cut4_ht[iCut4][1], cut6_mht[iCut6][0], cut6_mht[iCut6][1], cut6_lepVeto[iCut6][0], cut6_lepVeto[iCut6][1], cut6_jetMhtDPhi[iCut6], cut5_ak4Pt[iCut5][0], cut5_ak4Pt[iCut5][1]);
			...
			//PlotEntry2D plot2d = PlotEntry2D("BM1_1800sq_R0p99", hTemplate, varToPlot.c_str(), luminosity);
                        ...
                        // PlotEntry2D plot2d = PlotEntry2D("BM2_1900sq_R0p99", hTemplate, varToPlot.c_str(), luminosity);
                        // plot2d.AddInput("/opt/ppd/scratch/titterton/Analysis_boostedNmssmHiggs/flatTrees_MHT_2019_01_08/signal_mc/P3_1900sq_R0p99/flatTree.root", cutToApply.c_str(), 0.0219);

                        // PlotEntry2D plot2d = PlotEntry2D("BM2_1900sq_R0p384", hTemplate, varToPlot.c_str(), luminosity);
                        // plot2d.AddInput("/opt/ppd/scratch/titterton/Analysis_boostedNmssmHiggs/flatTrees_MHT_2019_01_08/signal_mc/P3_1900sq_R0p384/flatTree.root", cutToApply.c_str(), 0.0219);

                        //PlotEntry2D plot2d = PlotEntry2D("BM3_1800sq_R0p99", hTemplate, varToPlot.c_str(), luminosity);
                        //plot2d.AddInput("/opt/ppd/scratch/titterton/Analysis_boostedNmssmHiggs/flatTrees_MHT_2019_01_08/signal_mc/P5_1800sq_R0p99/flatTree.root", cutToApply.c_str(), 0.023290);

                        //PlotEntry2D plot2d = PlotEntry2D("BM3_1800sq_R0p384", hTemplate, varToPlot.c_str(), luminosity);
                        //plot2d.AddInput("/opt/ppd/scratch/titterton/Analysis_boostedNmssmHiggs/flatTrees_MHT_2019_01_08/signal_mc/P5_1800sq_R0p384/flatTree.root", cutToApply.c_str(), 0.023290);

                        // PlotEntry2D plot2d = PlotEntry2D("BM4_1800sq_R0p99", hTemplate, varToPlot.c_str(), luminosity);
                        // plot2d.AddInput("/opt/ppd/scratch/titterton/Analysis_boostedNmssmHiggs/flatTrees_MHT_2019_01_08/signal_mc/P6_1800sq_R0p99/flatTree.root", cutToApply.c_str(), 0.023290);

                        // PlotEntry2D plot2d = PlotEntry2D("BM4_1800sq_R0p384", hTemplate, varToPlot.c_str(), luminosity);
                        // plot2d.AddInput("/opt/ppd/scratch/titterton/Analysis_boostedNmssmHiggs/flatTrees_MHT_2019_01_08/signal_mc/P6_1800sq_R0p384/flatTree.root", cutToApply.c_str(), 0.023290);

                        // PlotEntry2D plot2d = PlotEntry2D("BM5_1900sq_R0p99", hTemplate, varToPlot.c_str(), luminosity);
                        // plot2d.AddInput("/opt/ppd/scratch/titterton/Analysis_boostedNmssmHiggs/flatTrees_MHT_2019_01_08/signal_mc/P7_1900sq_R0p99/flatTree.root", cutToApply.c_str(), 0.0219);

                        //PlotEntry2D plot2d = PlotEntry2D("BM5_1900sq_R0p384", hTemplate, varToPlot.c_str(), luminosity);
                        //plot2d.AddInput("/opt/ppd/scratch/titterton/Analysis_boostedNmssmHiggs/flatTrees_MHT_2019_01_08/signal_mc/P7_1900sq_R0p384/flatTree.root", cutToApply.c_str(), 0.0219);

                        //PlotEntry2D plot2d = PlotEntry2D("BM6_1900sq_R0p99", hTemplate, varToPlot.c_str(), luminosity);
                        //plot2d.AddInput("/opt/ppd/scratch/titterton/Analysis_boostedNmssmHiggs/flatTrees_MHT_2019_01_08/signal_mc/P8_1900sq_R0p99/flatTree.root", cutToApply.c_str(), 0.0219);

                        // PlotEntry2D plot2d = PlotEntry2D("BM6_1900sq_R0p384", hTemplate, varToPlot.c_str(), luminosity);
                        // plot2d.AddInput("/opt/ppd/scratch/titterton/Analysis_boostedNmssmHiggs/flatTrees_MHT_2019_01_08/signal_mc/P8_1900sq_R0p384/flatTree.root", cutToApply.c_str(), 0.0219);
			...
			// COMBINED OLD AND NEW!!!
                        ...
                        //double SMAX_Node1 = 197.1;
                        //double SMAX_Node2 = 133.1;
                        //std::vector<double> SN_Nodes = {51.9, 65.1, 78.3, 91.5, 104.7, 117.9, 131.1, 144.3, 157.5, 170.7, 183.9};
                        //MassRegionCuts MassCutsObject = MassRegionCuts("MassCutsV10vs09", S1_Node1, S1_Node2, SMAX_Node1, SMAX_Node2, SN_Nodes);
