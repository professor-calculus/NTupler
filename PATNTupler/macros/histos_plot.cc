...
int main(){
    ...
    std::string outputDir = "/opt/ppd/scratch/xap79297/Analysis_boostedNmssmHiggs/plots_2017_10_02/TEST_C/controlRegionEstimation/zoomInRatio/"; // where we are going to save the output plots (should include the samples name, and any important features)
    double luminosity = 36.773; // NB this is just a label for the plot. It should match the lumi of the histograms!
    ...
    std::vector<std::string> legendNames = {"S_control", "S_control_prediction"};
    ...
    plot.AddLatex(luminosity, "Data 2016");
...
void GetHistograms(std::map<std::string,TH1D*>& h_)
{
    ...
    histoNameVec.push_back("Data_JetHt2016_goldenJson");
